							...
							<div class="row">
								<div class="col-md-7">
									...
									<div class="form-group row">
										...
										<input type="file"  name="img"  id="img" class="form-control hidden" >
...
<script>
	...
	$("#form-banner").submit(function(event) {
		...
		if($.isNumeric(id)){
			var path = BaseUrl+"/api/admin/banner/update/"+id;
		}else{
			var path = BaseUrl+"/api/admin/banner-ads";
		}
				$.ajax({
							data: data,
							url: path,
	...
	function loadView(){
		...
		$.ajax({
					data: "",
					url: BaseUrl+"/api/admin/banner/view/"+id,
                    crossDomain: false,
                    method: 'GET',
                    complete: function(response){ 				
                        if(response.status == 200){
